...
def test_schematicity_vector_model(vector_mod, example_words):
    ...
    narsche.schematicity(model=vector_mod, words=words,
                         method='on-topic-ppn', topic='lamp')
    narsche.schematicity(model=vector_mod, words=words,
                         method='topic-relatedness', topic='lamp')
    narsche.schematicity(model=vector_mod, words=words,
                         method='pairwise-relatedness', pairs='adj')


def test_schematicity_network_model(network_mod, example_words):
    words = network_mod.keep_known(example_words)
    narsche.schematicity(model=network_mod, words=words,
                         method='on-topic-ppn', topic='lamp')
    narsche.schematicity(model=network_mod, words=words,
                         method='topic-relatedness', topic='lamp')
    narsche.schematicity(model=network_mod, words=words,
